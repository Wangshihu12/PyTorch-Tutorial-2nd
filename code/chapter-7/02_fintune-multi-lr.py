...
# @date       : 2022-06-24
# @brief      : finetune方法之不同学习率 - 使用多学习率策略进行模型微调
"""
import os
import numpy as np
import torch
...
start_epoch = -1        # 起始训练轮数（-1表示从第0轮开始）
...
# ImageNet预训练模型的标准化参数（均值）
norm_mean = [0.485, 0.456, 0.406]
# ImageNet预训练模型的标准化参数（标准差）
...
# =============================== 步骤4/5：优化器和学习率调度器 ===============================
# 多学习率策略：卷积层使用基础学习率，全连接层使用2倍学习率

# 获取全连接层参数的内存地址列表
fc_params_id = list(map(id, resnet18_ft.fc.parameters()))

# 过滤出非全连接层的参数（即卷积层参数）
# 使用filter函数，保留不在fc_params_id中的参数
base_params = filter(lambda p: id(p) not in fc_params_id, resnet18_ft.parameters())

# 创建SGD优化器，为不同参数组设置不同的学习率
optimizer = optim.SGD([
    {'params': base_params, 'lr': LR},                    # 卷积层参数使用基础学习率
    {'params': resnet18_ft.fc.parameters(), 'lr': LR*2}  # 全连接层参数使用2倍学习率
], momentum=0.9)  # 设置动量参数
