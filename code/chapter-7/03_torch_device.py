...
if __name__ == "__main__":
    ...
    # 返回值：int，表示可用的GPU数量
    # 如果没有GPU或CUDA不可用，返回0
    print("可用的GPU数量: {}".format(torch.cuda.device_count()))

    # 2. 获取当前活跃的CUDA设备索引
    # 返回值：int，当前设备索引（从0开始）
    # 如果没有GPU或CUDA不可用，会抛出异常
    print("当前活跃的GPU索引: ", torch.cuda.current_device())

    # 3. 获取CUDA设备的计算能力
    # @param device: 设备索引，None表示当前设备
    # 返回值：tuple，包含(major, minor)版本号
    # 例如：(7, 5)表示计算能力7.5
    print("CUDA设备的计算能力: ", torch.cuda.get_device_capability(device=None))

    # 4. 获取CUDA设备的名称
    # 返回值：str，GPU设备的名称
    # 例如："NVIDIA GeForce RTX 3080"
    print("CUDA设备的名称: ", torch.cuda.get_device_name())
    ...
    print("支持的CUDA架构列表: ", torch.cuda.get_arch_list())
    ...
    print("CUDA设备的详细属性: ", torch.cuda.get_device_properties(0))
    ...
    # free_memory: 可用显存（字节）
    # total_memory: 总显存（字节）
    print("CUDA设备的内存信息: ", torch.cuda.mem_get_info(device=None))
    ...
    # 返回值：str，内存使用情况的详细报告
    # 包括：已分配内存、缓存内存、空闲内存等
    print("CUDA设备的内存使用摘要: ", torch.cuda.memory_summary(device=None, abbreviated=False))
    ...
    # - 在内存不足时清理缓存
    # - 优化内存使用
    print("清空CUDA缓存: ", torch.cuda.empty_cache())
