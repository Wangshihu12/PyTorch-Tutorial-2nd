...
def main(args):
    ...
    # 根据参数选择数据增强策略
    if args.autoaug:
        # 使用torchvision的AutoAugment策略（基于ImageNet数据集优化的增强策略）
        auto_aug_list = torchvision.transforms.AutoAugment(transforms.AutoAugmentPolicy.IMAGENET)
        # 训练集数据增强：AutoAugment -> 调整尺寸 -> 随机裁剪 -> 转换为张量 -> 归一化
        train_transform = transforms.Compose([
            auto_aug_list,                           # 自动数据增强
            transforms.Resize(256),                  # 调整图像尺寸为256x256
            transforms.RandomCrop(input_size, padding=4),  # 随机裁剪为224x224，填充4像素
            transforms.ToTensor(),                   # 转换为PyTorch张量
            transforms.Normalize(normMean, normStd), # 归一化处理
        ])

        # 验证集数据预处理：调整尺寸 -> 转换为张量 -> 归一化（无数据增强）
        valid_transform = transforms.Compose([
            transforms.Resize(input_size),           # 调整图像尺寸为224x224
            transforms.ToTensor(),                   # 转换为PyTorch张量
            transforms.Normalize(normMean, normStd), # 归一化处理
        ])
    else:
        # 使用albumentations库进行数据增强（更灵活的数据增强库）
        train_transform = A.Compose([
            A.RandomScale(scale_limit=0.2, p=0.5),  # 随机缩放，缩放范围±20%，概率50%
            A.Resize(256, 256),                     # 调整图像尺寸为256x256
            A.RandomCrop(224, 224),                 # 随机裁剪为224x224
            A.Rotate(limit=30, p=0.5),             # 随机旋转，角度范围±30度，概率50%
            A.HorizontalFlip(p=0.5),               # 水平翻转，概率50%
            A.Normalize(normMean, normStd, max_pixel_value=255.),  # 归一化：基于0-1范围，像素值要求0-255，通过max_pixel_value=255实现整体数据变换为0-1
            ToTensorV2(),                           # 转换为PyTorch张量，仅数据转换，不会除以255
        ])

        # 验证集数据预处理：调整尺寸 -> 归一化 -> 转换为张量
        valid_transform = A.Compose([
            A.Resize(224, 224),                     # 调整图像尺寸为224x224
            A.Normalize(normMean, normStd, max_pixel_value=255.),  # 归一化处理
